#include <stdio.h>
void sort_by_profit(int weights[], int profits[], int indices[], int n) {
...
double fractional_knapsack(int n, int weights[], int profits[], int capacity, double solution[]) {
    ...
    sort_by_profit(weights, profits, indices, n);
    ...
    for (i = 0; i < n; i++) {
        if (current_weight + weights[i] <= capacity) {
            ...
            temp_solution[i] = (double)(capacity - current_weight) / weights[i];
